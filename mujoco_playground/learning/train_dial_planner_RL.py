...
def load_rl_policy(env_name: str, checkpoint_path: str):
  ...
  eval_env = (
      None if _VISION.value else registry.load(env_name, config=env_cfg)
  )
...
def main(argv):
  ...
  # stance to footstand
  policy_1, env_cfg = load_rl_policy("Go2Footstand", "Go2Footstand-20250907-195846")

  # footstand to handstand 
  policy_2, _ = load_rl_policy("Go2FlipRL", "Go2FlipRL-20250907-153240")
  
  # handstand to stance
  policy_3, _ = load_rl_policy("Go2RestoreRL", "Go2RestoreRL-20250907-164434")

  # Prepare for evaluation
  eval_env = (
      None if _VISION.value else registry.load("Go2Footstand", config=env_cfg)
  )
