...
def main(argv):
  ...
  def reverse_scan(rng_Y0_state, factor):
        ...
        rng, Y0, info = planner.reverse_once(state, rng, Y0, factor)
  ...
  config_dict = yaml.safe_load(
            open(get_example_path(f"{_ENV_NAME.value}.yaml"), "r"))
  dial_config = load_dataclass_from_dict(DialConfig, config_dict)
  planner = MBDPI(dial_config, env)
  YN = jp.zeros([dial_config.Hnode + 1, planner.nu])
  Y0 = YN
  # import pdb; pdb.set_trace()
  ...
  if _USE_WANDB.value and not _PLAY_ONLY.value:
    wandb.init(project="mjxrl", entity="yilang-liu-yale-university", name=exp_name)
  ...
  if _LOAD_CHECKPOINT_PATH.value is not None:
    ...
    ckpt_path = epath.Path("logs").resolve() / _LOAD_CHECKPOINT_PATH.value/ "checkpoints" # epath.Path(_LOAD_CHECKPOINT_PATH.value).resolve()
    if ckpt_path.is_dir():
  ...
  for t in range(env_cfg.episode_length):
    ...
    if t < 100:
      ...
      Y0 = planner.shift(Y0)
      state = jit_step(state, Y0[0])
      n_diffuse = dial_config.Ndiffuse
      if t == 100:
        n_diffuse = dial_config.Ndiffuse_init
        print("Performing JIT on Sampling")

      traj_diffuse_factors = (
                  planner.sigma_control * dial_config.traj_diffuse_factor ** (jp.arange(n_diffuse))[:, None]
              )
      (rng, Y0, _), info = jax.lax.scan(
                  reverse_scan, (rng, Y0, state), traj_diffuse_factors
              )
      # (rng, Y0, _), info =  reverse_scan((rng, Y0, state), traj_diffuse_factors[0])
