...
def flip_config() -> config_dict.ConfigDict:
  return config_dict.create(
      ctrl_dt=0.02,
      sim_dt=0.004,
      episode_length=200, # 500,
...
def flipRL_config() -> config_dict.ConfigDict:
  return config_dict.create(
      ctrl_dt=0.02,
      sim_dt=0.004,
      episode_length=200, # 500,
...
def restore_config() -> config_dict.ConfigDict:
  return config_dict.create(
      ctrl_dt=0.02,
      sim_dt=0.004,
      episode_length=200, # 500,
...
class Handstand(go2_base.Go2Env):
  ...
  def _get_termination(
    ...
    # return fall_termination | energy_termination
...
class Flip(go2_base.Go2Env):
  ...
  def __init__(
      self,
      config: config_dict.ConfigDict = flip_config(),
      config_overrides: Optional[Dict[str, Union[str, int, list[Any]]]] = None,
  ):
    super().__init__(
        xml_path=consts.FULL_FLAT_TERRAIN_XML.as_posix(),
  ...
  def _get_termination(
    ...
    energy = jp.sum(jp.abs(data.actuator_force) * jp.abs(data.qvel[6:]))
    energy_termination = energy > self._config.energy_termination_threshold
    return  fall_termination # fall_termination | contact_termination | energy_termination
  ...
  def _get_reward(
    ...
    return {
        "pos": self._cost_pos(data.qpos[:3]),
        "orientation": self._cost_orientation(data.qpos[3:7]),
        "joint_pos": self._cost_joint_pos(data.qpos[7:]),
        "base_lin_vel": self._cost_base_lin_vel(data.qvel[:3]),
        "base_ang_vel": self._cost_base_ang_vel(data.qvel[3:6]),
        "joint_vel": self._cost_joint_vel(data.qvel[6:]),
        "action_rate": self._cost_action_rate(action, info),
    }

  def _cost_pos(self, base_pos: jax.Array) -> jax.Array:
    error = base_pos - self._handstand_q[:3]
    weights = jp.array([2000, 2000, 20000])
    return - jp.sum(weights * jp.square(error))

  def _cost_orientation(self, quat: jax.Array) -> jax.Array:
    error = self._handstand_q[3:7] - quat
    return  -25000 * jp.sum(jp.square(error))
  
  def _cost_joint_pos(self, joint_pos: jax.Array) -> jax.Array:
    error = self._handstand_q[7:] - joint_pos
    return -300 * jp.sum(jp.square(error))
  
  def _cost_base_lin_vel(self, base_lin_vel: jax.Array) -> jax.Array:
    weights = jp.array([1000.0, 0.0001, 0.0001])
    return - jp.sum(weights * jp.square(base_lin_vel))
  
  def _cost_base_ang_vel(self, base_ang_vel: jax.Array) -> jax.Array:
    weights = jp.array([0.0001, 0.0001, 0.0001])
    return - jp.sum(weights * jp.square(base_ang_vel))

  def _cost_joint_vel(self, joint_vel: jax.Array) -> jax.Array:
    return -0.01 * jp.sum(jp.square(joint_vel))

  def _cost_action_rate(
      self, act: jax.Array, info: dict[str, Any]
  ) -> jax.Array:
    return -0.001 * jp.sum(jp.square(act - info["last_act"]))
...
class FlipRL(Handstand):
  """From footstand to handstand."""

  def _post_init(self) -> None:
    super()._post_init()
    self._init_q = jp.array(self._mj_model.keyframe("footstand").qpos)
    self._home_q = jp.array(self._mj_model.keyframe("home").qpos)
    self._default_pose = jp.array(self._mj_model.keyframe("home").qpos[7:])
    self._z_des = 0.55

    # Finetune remove when pretrain 
    geom_names = [
        "fl_calf_0",
        "fl_calf_1",
        "fr_calf_0",
    ...
    self._head_geom_id = np.array([1,2,3])
    self._unwanted_contact_geom_ids = np.append(self._unwanted_contact_geom_ids, self._head_geom_id)
  ...
  def reset(self, rng: jax.Array) -> mjx_env.State:
    ...
    rng, key = jax.random.split(rng)
    yaw = jax.random.uniform(key, (1,), minval=-3.14, maxval=3.14)
    quat = math.axis_angle_to_quat(jp.array([1, 0, 0]), yaw)
    new_quat = math.quat_mul(qpos[3:7], quat)
    qpos = qpos.at[3:7].set(new_quat)
    # d(xyzrpy)=U(-0.5, 0.5)
    qvel_nonzero = jp.zeros(self.mjx_model.nv)
    ...
    # randomize RR RL calf 
    rng, key = jax.random.split(rng)
    dcalf = jax.random.uniform(key, (12,), minval=-0.4, maxval=0.4)
    qpos = qpos.at[7:].set(qpos[7:]+dcalf)
    # qpos = qpos.at[16].set(qpos[16]+dcalf[1])
...
class RestoreRL(Handstand):
  """From handstand to stance."""
