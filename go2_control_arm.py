...
option_list = [
    ...
    # Robot Arm
    TestOption(name="arm up", id=20),
    TestOption(name="arm damp", id=21),
]

def ArmMessageHandler(msg: ArmString_):
    # print(msg)
...
if __name__ == "__main__":
    ...
    arm_subscriber = ChannelSubscriber("rt/arm_Feedback", ArmString_)
    arm_subscriber.Init(ArmMessageHandler, 10)

    arm_publisher = ChannelPublisher("rt/arm_Command", ArmString_)
    arm_publisher.Init()
    ...
    while True:
        ...
        if test_option.id == 0:
            ...
            data = '{"seq":4,"address":1,"funcode":7}'
            arm_cmd = ArmString_(data_=data)
            arm_publisher.Write(arm_cmd)
            print("arm zero control")
        elif test_option.id ==21:
            data = '{"seq":4,"address":1,"funcode":5,"data":{"mode":0}}'
            arm_cmd = ArmString_(data_=data)
            arm_publisher.Write(arm_cmd)
            print("arm turned off")
        time.sleep(1)
